-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj\hdlsrc\PWM_controller\Chart.vhd
...
-- 
-- Module: Chart
-- Source Path: PWM_controller/PWM Controller/Chart
-- Hierarchy Level: 1
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
...
USE work.PWM_Controller_pkg.ALL;
  ...
  BEGIN
    ...
    CASE is_Chart IS
      WHEN IN_HIGH =>
        ...
        END IF;
        IF (resize(count, 32) >= cast) AND (duty_unsigned < 255) THEN 
          b := '1';
        ELSE 
          b := '0';
        END IF;
        ...
        END IF;
      WHEN IN_LOW =>
        mul_temp_0 := (255 - duty_unsigned) * f_coeff_unsigned;
        IF mul_temp_0(39 DOWNTO 32) /= "00000000" THEN 
          cast_0 := X"FFFFFFFF";
        ELSE 
          cast_0 := mul_temp_0(31 DOWNTO 0);
        END IF;
        IF (resize(count, 32) >= cast_0) AND (duty_unsigned > 0) THEN 
          b_0 := '1';
        ELSE 
          b_0 := '0';
        END IF;
