-- -------------------------------------------------------------
-- 
-- File Name: hdl_prj\hdlsrc\EncoderRead\Channel_B_Interrupt.vhd
...
-- -------------------------------------------------------------
-- 
-- Module: Channel_B_Interrupt
-- Source Path: EncoderRead/dut_EncoderRead/Channel B Interrupt
-- Hierarchy Level: 1
-- 
-- -------------------------------------------------------------
LIBRARY IEEE;
...
END Channel_B_Interrupt;


ARCHITECTURE rtl OF Channel_B_Interrupt IS

  -- Signals
  SIGNAL is_Channel_B_Interrupt           : T_state_type_is_Channel_B_Interrupt;  -- uint8
  SIGNAL count_tmp                        : signed(31 DOWNTO 0);  -- int32
  SIGNAL reset_val                        : std_logic;
  SIGNAL count_reg                        : signed(31 DOWNTO 0);  -- int32
  SIGNAL is_Channel_B_Interrupt_next      : T_state_type_is_Channel_B_Interrupt;  -- enum type state_type_is_Channel_B_Interrupt (4 enums)
  SIGNAL reset_val_next                   : std_logic;
  SIGNAL count_reg_next                   : signed(31 DOWNTO 0);  -- int32

BEGIN
  Channel_B_Interrupt_1_process : PROCESS (clk, reset)
  BEGIN
    IF reset = '1' THEN
      is_Channel_B_Interrupt <= IN_I4;
    ELSIF clk'EVENT AND clk = '1' THEN
      IF enb = '1' THEN
        is_Channel_B_Interrupt <= is_Channel_B_Interrupt_next;
        reset_val <= reset_val_next;
        count_reg <= count_reg_next;
      END IF;
    END IF;
  END PROCESS Channel_B_Interrupt_1_process;
  Channel_B_Interrupt_1_output : PROCESS (is_Channel_B_Interrupt, A, B, reset_1, reset_val, enable, count_reg)
  ...
  BEGIN
    ...
    is_Channel_B_Interrupt_next <= is_Channel_B_Interrupt;
    count_reg_next <= count_reg;
    guard1 := '0';
    guard2 := '0';

    CASE is_Channel_B_Interrupt IS
      WHEN IN_I1 =>
        IF enable = '0' THEN 
          is_Channel_B_Interrupt_next <= IN_I3;
        ELSIF B = '1' THEN 
          IF A = B THEN 
            ...
            END IF;
            is_Channel_B_Interrupt_next <= IN_I2;
          ELSIF A /= B THEN 
      ...
      WHEN IN_I2 =>
        IF B = '0' THEN 
          IF A = B THEN 
            ...
            END IF;
            is_Channel_B_Interrupt_next <= IN_I1;
          ELSIF A /= B THEN 
        ...
        END IF;
      WHEN IN_I3 =>
        is_Channel_B_Interrupt_next <= IN_I4;
      WHEN OTHERS => 
        IF enable = '1' THEN 
    ...
    END IF;
    IF guard1 = '1' THEN 
      IF reset_1 /= reset_val_temp THEN 
        count_reg_next <= to_signed(16#00000000#, 32);
        reset_val_temp := reset_1;
        is_Channel_B_Interrupt_next <= IN_I1;
      END IF;
  ...
  END PROCESS Channel_B_Interrupt_1_output;
